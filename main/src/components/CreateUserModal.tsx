...
export const CreateUserModal: React.FC<CreateUserModalProps> = ({ isOpen, onClose, onSuccess, role }) => {
    ...
    React.useEffect(() => {
        if (role === 'student' && user?.role === 'coordinator' && user.program) {
            setFormData(prev => ({ ...prev, program: user.program! }));
        }
    }, [role, user]);
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Create New ${role.charAt(0).toUpperCase() + role.slice(1)}`}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Only show program select if not a coordinator creating a student (since it's auto-assigned) 
                    OR allow them to see it but disabled? 
                    Better to show it disabled so they know. */}
                <Select
                    label="Program"

...
export const Messages: React.FC = () => {
    ...
    // const [selectedConversationId, setSelectedConversationId] = useState<string | null>(null); // Replaced with URL param
    ...
    // Fetch available tasks and documents for auto-linking
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [tasksRes, docsRes] = await Promise.all([
                    ...
                    fetch(`${API_BASE_URL}/document`, { headers: { 'Authorization': `Bearer ${token}` } })
                ]);
                ...
                console.error("Failed to fetch tasks/docs for linking", error);
            }
        };
        fetchData();
    }, [token]);
    ...
    useEffect(() => {
        ...
        const hasUnread = messages.some(msg => {
            ...
            if (msg.sender._id === user._id) return false; // Own messages are read

            // Check if message belongs to selected conversation
            if (msg.receiverModel === 'Conversation') {
                return msg.receiver._id === selectedConversationId;
            } else {
                return msg.sender._id === selectedConversationId;
            }
        });
        ...
        if (hasUnread) {
            ...
            setMessages(prev => prev.map(msg => {
                const isRelevant = msg.receiverModel === 'Conversation'
                    ? msg.receiver._id === selectedConversationId
                    : msg.sender._id === selectedConversationId;

                if (isRelevant && !msg.isRead && msg.sender._id !== user._id) {
    ...
    }, [selectedConversationId, messages, user, token, conversations]);
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (imageFile) {
                ...
                const uploadResponse = await fetch(`${API_BASE_URL}/message/upload`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    },
    ...
    const renderMessageContent = (content: string) => {
        ...
        const taskMap = new Map(availableTasks.map((t: any) => [t.title.toLowerCase(), t._id]));
        const docMap = new Map(availableDocuments.map((d: any) => [d.documentName.toLowerCase(), d._id]));

        const universalLinks: Record<string, string> = {
            "documents": user?.role === 'student' ? "/upload-documents" : "/documents",
            "document": user?.role === 'student' ? "/upload-documents" : "/documents",
            "tasks": user?.role === 'student' ? "/my-tasks" : "/tasks",
            "task": user?.role === 'student' ? "/my-tasks" : "/tasks",
            "dashboard": "/dashboard",
            "home": "/dashboard",
            "students": "/students",
        ...
        // Combine all phrases
        const phrases = [
            ...availableTasks.map((t: any) => t.title),
            ...availableDocuments.map((d: any) => d.documentName),
            ...Object.keys(universalLinks)
        ].filter(Boolean).sort((a, b) => b.length - a.length); // Longest first

        if (phrases.length === 0) return <p className="text-xs md:text-sm leading-relaxed">{content}</p>;
        ...
        return (
            <p className="text-xs md:text-sm leading-relaxed">
                {parts.map((part, i) => {
                    ...
                    if (universalLinks[lowerPart]) {
                        return (
                            <span
                                key={i}
                                className="text-blue-200 underline cursor-pointer hover:text-white font-semibold"
                                onClick={(e) => {
                                    e.stopPropagation();
                                    navigate(universalLinks[lowerPart]);
                                }}
                                title={`Go to ${part}`}
                            >
                                {part}
